...
def addUser(event, context):
    ...
    # if(checkOrg({'orgid':event['orgid']}) == False):
    ...
    with conn.cursor() as cur:
        ...
        returnresult = '{"status":404, "message":"Unable to store Driver in database"}'    
        ...
        for row in result:
            ...
            returnresult = getuser({'userid' : row, 'status' : state})
            logAudit({'user_id': row, 'operation' : 'Driver Application', 'state' : 'applied', 'message' : str(event) })
